...
SELECT loan_id,account_id,amount-payments as debt from bank.loan where status = 'B' and amount-payments > 1000 ORDER  BY debt DESC;
...
SELECT * FROM bank.trans;
SELECT max(amount) as max,  min(amount) as min from bank.trans where amount  <> 0;
...
SELECT * from bank.account;
select * , substring(date,3,2) AS year from bank.account;
-- ==================================================
...
select card_id, type, DATE_FORMAT(CONVERT(SUBSTRING_INDEX(issued,' ',1), date),'%Y') AS year FROM bank.card where type = 'gold';
-- 2. When was the first gold card **issued**? (Year)
select card_id, DATE_FORMAT(CONVERT(SUBSTRING_INDEX(issued, ' ',1),date), '%y-%m-%d') AS year
from bank.card where type = 'gold' order by year asc limit 1; 
-- 3. Get issue date as:
select card_id, DATE_FORMAT(CONVERT(SUBSTRING_INDEX(issued, ' ', 1) date), '%M,%D,%Y') AS American, DATE_FORMAT(CONVERT(SUBSTRING_INDEX(issued, ' ', 1) date), '%d of %M of %Y') AS Spanish;
...
SELECT SUM(ISNULL(amount)) FROM bank.trans;
...
SELECT SUM(ISNULL(amount)) FROM bank.trans;
SELECT SUM(ISNULL(k_symbol)=1 OR k_symbol IN ("", " "))  FROM bank.trans;
SELECT SUM(NOT ISNULL(k_symbol) OR k_symbol IN ("", " "))  FROM bank.trans
WHERE k_symbol IN ("", " ");
SELECT SUM(amount IS NULL OR amount IN (" ", "")) AS Transactions FROM bank.trans;
...
SELECT * FROM bank.district
WHERE A2 regexp '[a-yA-Z]{5}';

SELECT * FROM bank.district
WHERE A2 regexp 'v$';  -- + -> after letter means it appears at least once, * -> appears any amount 
